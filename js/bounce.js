...
Bounce.Game.prototype._addScore = function() {
    ...
    this.game.bounce.winningScore = STD;
    text = this.add.existing(new CenteredText(this.game, "std09000", style, bounds));
    text.setShadow(
        style.shadowOffsetX, style.shadowOffsetY,
        style.shadowColor, style.shadowBlur);

    this.input.onDown.addOnce(function startCount() {
        this.time.events.loop(Phaser.Timer.SECOND / 9, function updateScore() {
            text.text = "std09" + String(++this.game.bounce.score).padStart(3, '0');
            if (this.game.bounce.score == this.game.bounce.winningScore) {
